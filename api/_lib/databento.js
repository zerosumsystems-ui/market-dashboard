...
// Supports raw_symbol→instrument_id and instrument_id→raw_symbol
...
export async function getRange({ schema, symbols, start, end, dataset }) {
  ...
  // Always use CSV encoding (proven to work, JSON mappings unreliable via HTTP)
  const body = new URLSearchParams({
    dataset: ds,
    schema,
  ...
  // Fetch data; for multi-symbol or ALL_SYMBOLS, resolve instrument_id→symbol in parallel
  const needsSymbolResolution = !isSingleKnownSymbol;
  const [r, preResolvedMap] = await Promise.all([
    ...
    needsSymbolResolution && !isAllSymbols
  ...
  // Detect format: if open > 1e8, values are fixed-point nanoseconds; else dollars
  const sampleOpen = Number(rows[0].open);
  const isPretty = sampleOpen < 1e8;

  // For ALL_SYMBOLS: collect unique instrument_ids from CSV, then resolve to symbols
